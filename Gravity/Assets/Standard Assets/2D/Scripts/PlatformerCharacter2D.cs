...
namespace UnityStandardAssets._2D
{
    public class PlatformerCharacter2D : MonoBehaviour
    {
		...
		public void Move(float move, float moveV, bool crouch, bool swap)
        {
            ...
            if (m_Grounded || m_AirControl)
            {
				...
				if (Physics2D.gravity.x != 0) {
					m_Rigidbody2D.velocity = new Vector2(m_Rigidbody2D.velocity.x, moveV*m_MaxSpeed);
				...
				if (Physics2D.gravity.y < 0){  //zero degrees
				...
				else if (Physics2D.gravity.x > 0){ //90 degrees
				...
				else if (Physics2D.gravity.y > 0){ //180 degrees
					if (move > 0 && m_FacingRight)
					{
						Flip();
					}
				...
				else if (Physics2D.gravity.x < 0){ //270 degrees
					if (moveV < 0 && m_FacingRight)
					{
						Flip();
					}
			...
			if (m_Grounded && swap && m_Anim.GetBool("Ground"))
            {
				...
				if (Physics2D.gravity.y > 0) {
					...
					Physics2D.gravity = new Vector2(Physics2D.gravity.x * -1, 0);
